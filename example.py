...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        font = QFont()
        font.setPointSize(48)  # Make the emoji bigger
        self.emoji_status_label.setFont(font)
        # Align the emoji to the right within its layout space
    ...
    def set_status_emoji(self, state):
        """Sets the status label to the corresponding emoji."""
    ...
    def update_data(self, measured_value, temperature, status):
        """Main slot for receiving new data and updating the UI state."""
        if status == 0:
            self.set_status_emoji("ok")
        else:
    ...
    def parse_status(self, status):
        """Parses the integer status code into a human-readable string."""
        if status == 0:
            return "Status: OK"
